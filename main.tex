...
\begin{itemize}
    ...
    \item At $\lambda=1/2$: Each longer return is included but weighted less: 1-step gets $1/2^0=1$; 2-step gets $1/2^1=0.5$; 3-step $1/2^2=0.25$, etc.
\end{itemize}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.5\linewidth]{epsilon_greedy.png}
...
\subsection{Deep Deterministic Policy Gradient (DDPG) Notes}
...
SAC is a model-free, off-policy,actor-critic algorithm that aims to find a policy that not only maximizes the reward but also has high entropy. This encourages exploration and leads to more robust, stable policies.
